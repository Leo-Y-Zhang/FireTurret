...
| **Renaming the Python package during the 2026-08-03 repo rename** *(rejected, then reversed on 2026-08-11)* | The package, the CLI, `*.triton.json` session files and the ROS frame ids are interfaces, not labels, and a cosmetic rename is not worth breaking every import, saved calibration and TF subscriber. Outweighed a week later by a tree that read as though the rename had never happened; the break was taken. See "A note on the name" in the README. |
