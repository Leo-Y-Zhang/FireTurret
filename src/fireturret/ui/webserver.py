...
<h1>FIRETURRET</h1><div id=warn></div>
...
def serve(
    ...
    ``water_enabled`` is True — the same safe default as ``fireturret run``. The
    ...
    print(f"FireTurret web interface on http://{host}:{port}  (Ctrl-C to stop)")
