...
DEFAULT_CONFIG = FireTurretConfig()

# --- calibration as data: load/save a FireTurretConfig to a JSON file --------------
...
def config_to_dict(cfg: FireTurretConfig) -> dict:
...
def config_from_dict(d: dict) -> FireTurretConfig:
    ...
    cfg = FireTurretConfig(**{key: cls(**groups[key]) for key, cls in _SUBCONFIGS.items()})
...
def plugin_config(cfg: FireTurretConfig, name: str):
    """The registered plugin sub-config `name`, or None if this config has none.

    Plugins cannot add FIELDS to `FireTurretConfig` — it is a fixed dataclass — so
...
def load_config(path) -> FireTurretConfig:
