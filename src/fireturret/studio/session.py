...
"""The Studio Session document: the current FireTurretConfig + SimScenario, with
...
class Session(QObject):
    ...
    def __init__(self, config: FireTurretConfig | None = None,
                 scenario: SimScenario | None = None, parent=None) -> None:
        super().__init__(parent)
        self._config = config or FireTurretConfig()
        self._scenario = scenario or SimScenario()
        self._dirty = False
        self.undo_stack = QUndoStack(self)

    @property
    def config(self) -> FireTurretConfig:
        return self._config
