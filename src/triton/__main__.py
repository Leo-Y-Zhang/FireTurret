...
"""FireTurret CLI.
  ...
  triton sim [--scenario NAME] [--seed N] [--headless] [--frames N] [--record out.mp4]
  ...
  triton run --source 0|video.mp4 [--port COM3] [--config turret.json] [--arm-water]
      ...
      NullRig gives a dry-run aim preview. --config loads a calibrated TritonConfig.

  triton selftest --port COM3 [--arm-water]
  ...
  triton web [--source 0|video.mp4|sim] [--port COM3] [--http-port 8000] [--expose]
  ...
  triton fit shots.csv
      Calibrate velocity_coeff and drag_k from measured test shots
      (CSV rows: pump_pct,elevation_deg,measured_range_m).

  triton studio
      Launch Triton Studio, the desktop engineering workbench
      (needs the [desktop] extra: pip install triton[desktop]).
"""
...
def _cmd_sim(args: argparse.Namespace) -> int:
    ...
    if args.report:
        ...
        write_report(report.telemetry, args.report, lo, hi,
                     title=f"Triton mission - {args.scenario}")
...
def _cmd_fit(args: argparse.Namespace) -> int:
    ...
    print("update src/triton/config.py JetConfig with:")
...
def _cmd_studio(args: argparse.Namespace) -> int:
    ...
    never depends on PySide6 (needs the `triton[desktop]` extra)."""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(prog="triton", description=__doc__)
    ...
    p_sim.add_argument("--config", default=None, help="load a calibrated TritonConfig JSON (Studio export or hand-edited)")
    ...
    p_run.add_argument("--config", default=None, help="load a calibrated TritonConfig JSON")
    ...
    p_web.add_argument("--config", default=None, help="load a calibrated TritonConfig JSON")
    ...
    p_selftest.add_argument("--arm-water", action="store_true",
                            help="also pulse pump/valve briefly (asks for confirmation; "
                                 "default is dry)")
    p_selftest.add_argument("--config", default=None, help="load a calibrated TritonConfig JSON")
    p_selftest.set_defaults(func=_cmd_selftest)

    p_studio = sub.add_parser(
        "studio", help="launch the desktop engineering workbench (needs triton[desktop])"
    )
