...
- A **session** is ``*.triton.json``: config + scenario.
...
from ..config import TritonConfig, config_from_dict, config_to_dict
...
def save_session(path, config: TritonConfig, scenario: SimScenario) -> None:
    data = {
        "version": 1,
...
def load_session(path) -> tuple[TritonConfig, SimScenario]:
...
def save_run(dir_path, name: str, config: TritonConfig, scenario: SimScenario,
             seed: int, report) -> str:
