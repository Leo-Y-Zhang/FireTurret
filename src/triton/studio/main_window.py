...
"""Triton Studio main window: the demoable spine — edit a model parameter, Run a
...
class MainWindow(QMainWindow):
    ...
    def _build_menu(self) -> None:
        ...
        about_act = QAction("About Triton Studio", self)
    ...
    def open_session(self) -> None:
        path, _ = QFileDialog.getOpenFileName(
            self, "Open session", "", "Triton session (*.triton.json)")
        if path:
    ...
    def save_session_as(self) -> None:
        path, _ = QFileDialog.getSaveFileName(
            self, "Save session", "", "Triton session (*.triton.json)")
        if path:
    ...
    def _update_title(self) -> None:
        self.setWindowTitle(("*" if self.session.dirty else "") + "Triton Studio")
    ...
    def _about_text(self) -> str:
        ...
        try:
            triton_v = version("triton")
        except PackageNotFoundError:
            triton_v = "0.1.0"
        return (
            "<b>Triton Studio</b><br>Fire-suppression turret engineering workbench"
            f"<br><br>triton {triton_v}<br>PySide6 {PySide6.__version__}"
            f"<br>pyqtgraph {pyqtgraph.__version__}<br>default seed: 7"
        )

    def _show_about(self) -> None:
        QMessageBox.about(self, "About Triton Studio", self._about_text())
    ...
    def _report_error(self, summary: str, details: str) -> None:
        ...
        box.setWindowTitle("Triton Studio — error")
        box.setText(summary)
