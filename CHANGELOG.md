...
- **Renamed the Python package from `triton` to `fireturret`** (2026-08-11,
  `e4e3dde`). This reverses the *Unchanged (deliberately)* entry below, and the
  reasoning there was not wrong so much as outweighed: clicking into the
  repository gave no sign the rename had happened, because the distribution
  name, the console script, the package directory, the config class and the
  window title all still said Triton. 100 files updated and the package
  directory moved, so the compatibility break that entry was protecting was
  taken rather than avoided - a saved `*.triton.json` session and a subscriber on
  the `triton/...` TF frames are both on their own.

  It closes the *Known issue* below outright: `fireturret` is not taken on PyPI,
  so the distribution no longer shares a name with anything, let alone with a
  package PyTorch pulls in.

  It also went through the dated design record under `docs/specs/`, which the
  entry below says should not happen. The filename still carries the old name;
  the contents no longer do. Recorded rather than re-edited, because editing it
  again to match either statement would be the same mistake a second time.
...
*Superseded on 2026-08-11 - see the package rename under Changed.*
...
*Resolved on 2026-08-11 by the package rename under Changed.*
